...
public class WdElementQuietWaitingDriver implements IElementQuietWaitingDriver {
    ...
    public <T> boolean condition(Function<IInteractiveElement, T> function, long timeoutInSeconds) {
        return executeAndSuppressException(() -> elementWaitingDriver.condition(function, timeoutInSeconds));
    }

    @Override
    public <T> boolean condition(Function<IInteractiveElement, T> function, long timeoutInSeconds, String message) {
        return executeAndSuppressException(() -> elementWaitingDriver.condition(function, timeoutInSeconds, message));
    }

    @Override
    public <T> boolean condition(Function<IInteractiveElement, T> function, long timeout, TemporalUnit unit) {
        return executeAndSuppressException(() -> elementWaitingDriver.condition(function, timeout, unit));
    }

    @Override
    public <T> boolean condition(Function<IInteractiveElement, T> function, long timeout, TemporalUnit unit, String message) {
        return executeAndSuppressException(() -> elementWaitingDriver.condition(function, timeout, unit, message));
    }
}
